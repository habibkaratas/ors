...
### Hazır paketten (önerilen)

Depoda kök dizinde hazır bir **`ors.vsix`** bulunur. Kurmak için:

1. `ors.vsix` dosyasını indir (bu depodan).
2. VSCode → **Extensions** paneli → sağ üstteki `···` → **Install from VSIX…** → `ors.vsix` seç.
3. `Ctrl+Shift+P` → **Developer: Reload Window**.
4. Sol activity bar'da **Örs** ikonu görünür.

> Güncelleme: yeni bir `ors.vsix` geldiğinde önce mevcut sürümü **Uninstall** edip
> yeniden kur (VSCode aynı sürümü otomatik yenilemez).

Ön koşul olarak Ollama'nın çalışıyor olması gerekir — aşağıdaki **Ön koşul: Ollama** bölümüne bak.
...
```bash
npm run package     # kök dizinde ors.vsix üretir (vsce ile)
```

Üretilen `ors.vsix`, yukarıdaki **Install from VSIX** adımıyla herhangi bir VSCode'a kurulabilir.
