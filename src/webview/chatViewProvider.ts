...
import type { ApprovalPreview, DiagnosticEntry, ToolContext } from "../tools/types";
...
function toDiagnosticEntry(d: vscode.Diagnostic, uri: vscode.Uri): DiagnosticEntry {
  return {
    severity:
...
function transcriptToMarkdown(transcript: TranscriptItem[]): string {
  const lines: string[] = ["# Örs Sohbeti", ""];
  for (const it of transcript) {
    if (it.kind === "user") lines.push("## 🧑 Sen", "", it.text, "");
    else if (it.kind === "assistant") lines.push("## 🤖 Ajan", "", it.text, "");
    else if (it.kind === "tool")
      lines.push(
        `> 🔧 **${it.name}** — ${it.summary} → ${it.ok ? "OK" : "HATA"}` +
          (it.detail ? `: ${it.detail}` : ""),
        ""
      );
    else lines.push(`> ${it.level === "error" ? "⚠️" : "ℹ️"} ${it.text}`, "");
  }
  return lines.join("\n");
}
...
export class ChatViewProvider implements vscode.WebviewViewProvider {
  ...
  private pendingExternal?: { text: string; autoSend: boolean };
  ...
  resolveWebviewView(view: vscode.WebviewView): void {
    ...
    view.webview.onDidReceiveMessage((m: HostBoundMessage) => this.onMessage(m));
    view.onDidDispose(() => {
  ...
  newChatCommand(): void {
    ...
    index.push({ id: newId, name: "Yeni sohbet", active: false, autoName: true });
  ...
  private async onMessage(m: HostBoundMessage): Promise<void> {
    switch (m.type) {
      ...
      case "getEditorContext":
        ...
        break;
      case "openFile":
        await this.openFileAt(m.path, m.line);
        break;
      case "exportChat":
  ...
  private async openFileAt(p: string, line?: number): Promise<void> {
    try {
      const abs = path.isAbsolute(p) ? p : path.join(this.rootDir(), p);
      ...
      this.error(`Dosya açılamadı: ${p}`);
    }
  }

  private async exportChat(): Promise<void> {
    const s = this.active;
    const md = transcriptToMarkdown(s.transcript);
    const uri = await vscode.window.showSaveDialog({
      filters: { Markdown: ["md"] },
      defaultUri: vscode.Uri.file(path.join(this.rootDir(), "ors-sohbet.md")),
    });
    if (!uri) return;
    await vscode.workspace.fs.writeFile(uri, Buffer.from(md, "utf8"));
    this.info(`Sohbet dışa aktarıldı: ${uri.fsPath}`);
  }

  public async actOnSelection(kind: "explain" | "fix" | "send"): Promise<void> {
    const editor = vscode.window.activeTextEditor;
    if (!editor || editor.selection.isEmpty) {
      vscode.window.showInformationMessage("Önce editörde kod seç.");
      return;
    }
    const sel = editor.document.getText(editor.selection);
    const file = vscode.workspace.asRelativePath(editor.document.uri);
    const block = "```" + editor.document.languageId + "\n" + sel + "\n```";
    let text: string;
    let autoSend = true;
    if (kind === "explain") {
      text = `Şu seçili kodu sade bir dille açıkla (${file}):\n\n${block}`;
    } else if (kind === "fix") {
      text = `Şu seçili kodda hata/sorun varsa bul ve düzelt (${file}):\n\n${block}`;
    } else {
      text = `${file} dosyasından seçili kod:\n\n${block}\n\n`;
      autoSend = false;
    }
    await this.deliverExternalPrompt(text, autoSend);
  }

  private async deliverExternalPrompt(text: string, autoSend: boolean): Promise<void> {
    this.pendingExternal = { text, autoSend };
    await vscode.commands.executeCommand("ors.chat.focus");
    if (this.viewReady) await this.flushExternal();
  }

  private async flushExternal(): Promise<void> {
    const pe = this.pendingExternal;
    if (!pe) return;
    this.pendingExternal = undefined;
    if (pe.autoSend) await this.handlePrompt(pe.text);
    else this.post({ type: "prefill", text: pe.text });
  }
  ...
  private async runOne(s: SessionRuntime, text: string, images?: string[]): Promise<void> {
    const cfg = this.readConfig();
    if (!cfg.model) {
      this.emitError(s, "Model seçili değil. Üstteki menüden bir model seç (Ollama çalışıyor olmalı).");
      return;
    }
    if (images?.length) {
    ...
    try {
      ...
      this.postTo(s.id, { type: "undoAvailable", available: s.checkpoints.hasUndo() });
      this.persist(s);
    }
  }
  ...
  async setHostCommand(): Promise<void> {
    ...
    const pick = await vscode.window.showQuickPick(
      [...hosts.map((h) => (h === current ? `● ${h}` : `  ${h}`)), ADD],
      { placeHolder: "Ollama host seç veya ekle" }
    );
    if (!pick) return;
    let url: string;
    if (pick === ADD) {
      const input = await vscode.window.showInputBox({
        ...
        value: "http://",
        validateInput: (v) => (/^https?:\/\/.+/.test(v) ? null : "http(s):// ile başlamalı"),
      });
      if (!input) return;
      url = input.trim();
      if (!hosts.includes(url)) {
        await cfg.update("hosts", [...hosts, url], vscode.ConfigurationTarget.Global);
      }
    } else {
      url = pick.replace(/^[●\s]+/, "");
    }
  ...
  private async warnIfNoVisionSupport(model: string, s: SessionRuntime): Promise<void> {
    if (!model) return;
    ...
    if (!supported) {
      this.postTo(s.id, {
        type: "error",
        text: `Uyarı: '${model}' modeli görsel (vision) desteklemiyor. ` +
  ...
  private makeEvents(s: SessionRuntime): AgentEvents {
    return {
      ...
      toolStart: (id, name, summary) => this.emitToolStart(s, id, name, summary),
  ...
  private emitStatus(s: SessionRuntime, state: "idle" | "thinking" | "running"): void {
    const label =
      state === "running" && s.currentTool ? s.currentTool :
      state === "thinking" ? "düşünüyor" :
      undefined;
    this.postTo(s.id, { type: "status", state, label, ctxPct: this.estimateCtxPct(s) });
  }
  ...
  private estimateCtxPct(s: SessionRuntime): number | undefined {
    ...
    const totalChars = s.transcript.reduce((sum, item) => {
      if (item.kind === "user" || item.kind === "assistant") return sum + item.text.length;
      if (item.kind === "tool") return sum + item.summary.length + item.detail.length;
      return sum;
    }, 0);
  ...
  private emitToolStart(s: SessionRuntime, id: string, name: string, summary: string): void {
    s.currentTool = name;
  ...
  private emitToolEnd(s: SessionRuntime, id: string, ok: boolean, detail: string): void {
  ...
  private makeApproval(s: SessionRuntime): ApprovalGate {
    return {
      request: (tool, args, preview) => this.requestApproval(s, tool, args, preview),
    };
  }
  ...
  private removePrompt(s: SessionRuntime, id: string): void {
    s.pendingPrompts = s.pendingPrompts.filter(
      (p) => !((p.type === "approvalRequest" || p.type === "askUser") && p.id === id)
    );
  }
  ...
  private async rememberCommandAllow(
    ...
    if (list.some((p) => p.trim().toLowerCase() === prefix.toLowerCase())) return;
    await cfg.update(
      "commandAllowlist",
      [...list, prefix],
  ...
  private toolContext(s: SessionRuntime): ToolContext {
    ...
    return {
      get workspaceRoot() { return self.worktreeOverride ?? self.rootDir(); },
      resolvePath: (rel) =>
        makeResolvePath(self.worktreeOverride ?? self.rootDir(), workspaceOnly)(rel),
      ...
      getDiagnostics: (absPath) => {
        ...
        if (uri) {
          return vscode.languages.getDiagnostics(uri).map((d) => toDiagnosticEntry(d, uri));
        }
        return vscode.languages.getDiagnostics().flatMap(([u, diags]) =>
          diags.map((d) => toDiagnosticEntry(d, u))
        );
      },
      ...
      spawnSubAgent: async (task, toolNames, token) => {
        ...
        const tools = toolNames.length > 0
          ? toolNames.map((n) => this.registry.get(n)).filter((t): t is NonNullable<typeof t> => t !== undefined)
          : this.registry.all();
        ...
        const events: AgentEvents = {
          status: () => {},
          assistantStart: () => {},
          assistantToken: (t) => { output.push(t); },
          assistantEnd: () => {},
          assistantDiscard: () => {},
          toolStart: () => {},
          toolEnd: () => {},
        ...
        const sub = this.buildAgent(subRegistry, this.toolContext(s), events, approval, () => agentCfg);
      ...
      askUser: (title, options) =>
        new Promise<string>((resolve) => {
          ...
          const payload: WebviewBoundMessage = { type: "askUser", id, title, options };
          s.pendingPrompts.push(payload);
          const timer = setTimeout(() => {
      ...
      lspExecute: (command, filePath, line, character, extra) => {
        ...
        if (extra !== undefined) {
          return vscode.commands.executeCommand(command, uri, position, extra) as Promise<unknown>;
        }
        return vscode.commands.executeCommand(command, uri, position) as Promise<unknown>;
      },
      ...
      lspApplyRename: async (filePath, line, character, newName) => {
        ...
        if (!edit) return "Rename provider yanıt vermedi veya sembol bulunamadı.";
  ...
  private buildAgent(
    ...
    return new Agent(
      ...
      approval,
      cfg,
      { platform: process.platform, shell: process.platform === "win32" ? "powershell" : "sh" },
      () => this.memory.list()
    );
  }
  ...
  private renderSession(s: SessionRuntime): void {
    ...
    if (s.busy && s.streamingItem && items[items.length - 1] === s.streamingItem) {
      items.pop();
  ...
  private async handleFilePicker(): Promise<void> {
    ...
    const binaryExts = new Set(["pdf", "doc", "docx", "xls", "xlsx", "ppt", "pptx", "zip", "rar", "7z", "tar", "gz", "exe", "dll", "bin", "dat"]);
    ...
    try {
      ...
      if (imageExts.has(ext)) {
        ...
        this.post({ type: "fileAttached", name, content: `[Görsel ek: ${uri.fsPath}]`, isImage: true, dataUrl });
      } else if (binaryExts.has(ext)) {
        this.error(`İkili dosya eklenemez: ${name}. Lütfen metin dosyası seçin.`);
      } else {
  ...
  private handleEditorContext(): void {
    const editor = vscode.window.activeTextEditor;
    if (!editor) { this.info("Aktif editör yok."); return; }
